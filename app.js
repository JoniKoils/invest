...
mongo.connect('mongodb://c25931_football_inv_bot_na4u_ru:PuRlaSasdosoh23@mongo1.c25931.h2,mongo2.c25931.h2,mongo3.c25931.h2/c25931_football_inv_bot_na4u_ru?replicaSet=MongoReplica'); //

const ADMINS = [760982306,871795367];
const CHANNEL_INFO = [-1001269534781];
...
const RM_admin = {
  inline_keyboard: [
    ...
    [{ text: "✏️ Лимит количества клубов", callback_data: "admin_limit" }],
...
const bot = new Telegram('1232406787:AAGGfk9gJsYSYUqbPP8CLMtHL7Cxgk0pG6U', { polling: true });
      ...
      if (postfix) {
        if (postfix[0] == "C") {
          ...
          if (c) {
            ...
            return bot.sendPhoto(message.chat.id, `c${c.tree_id}.png`, { caption: `✅ Вы успешно активировали чек и получили клуб - <b>${(trees.find((a) => a.id == c.tree_id)).name}</b>`, parse_mode: "HTML", })
            // return bot.sendPhoto(message.chat.id, `c${c.tree_id}.png`, { caption: `✅ Вы успешно активировали чек и получили клуб - <b>${(trees.find((a) => a.id == c.tree_id)).name}</b>`, parse_mode: "HTML", })
          }
        }
      }
...
🏝 <b>Чат</b> 👉 @chatbotaa
💳 <b>Выплаты</b> 👉 @viplatiotbota
📢 <b>Новости</b> 👉 @novostiotbotaa`, {
        parse_mode: "HTML",
        reply_markup: {
          keyboard: $menu,
  ...
  if (state[uid] == 7771222 && ADMINS.indexOf(message.from.id) !== -1) {
    ...
    return message.send(`Лимит на Клубы в ${text} единиц установлен!`, { reply_markup: RM_admin_return });
  }
  ...
  if (state[uid] == 7772 && ADMINS.indexOf(message.from.id) !== -1 && text != "0") {
    ...
    kb.inline_keyboard.push([{ text: "➕ Выдать клуб", callback_data: "giveTree_" + u.id }, { text: "❌ Забрать клуб", callback_data: "takeTree_" + u.id }])
...
⚽️ Мои клубы: <b>${user.trees.length}</b>
  ...
  if (message.user.menu.startsWith('amountQiwi')) {
    ...
    if (message.text <= message.user.outbalance) {
      ...
      if (message.text + message.user.payout >= message.user.deposit * 4) {
        message.send(`Вы вывели из бота в 4 раза больше, чем вложили\nВаш аккаунт обнулён`);
      ...
      return message.send(`<b>👇 Ваш подарок здесь 👇</b>\n`, {
        ...
        reply_markup: {
          inline_keyboard: [
            ...
            [{ text: "🎁 Получить подарок 🎁", callback_data: "game_prize" }]
          ]
        }
      });
    }

    if (message.text == '♻️ Обменник') {
      return message.send(`Здесь Вы сможете обменять <b>⚽️ голов</b> на <b>₽ рубли</b>

1000 голов ⚽️ = 1 рубль
Минимальная сумма обмена: 1000 ⚽️ голов

⚽️ <b>У вас голов:</b> ${message.user.fetuses.toFixed(2)}
...
⚽️ <b>Мои клубы:</b> ${message.user.trees.length}
...
🤑 <b>Выведено:</b> ${message.user.payout.toFixed(2)}₽`, {
        ...
        reply_markup: {
          inline_keyboard: [
            ...
            [{ text: "♻️ Реинвест", callback_data: "reinvest" }, { text: "⚽️ Мои клубы", callback_data: "trees:totalMy" }],
          ]
        }
      });
    }
...
🔗 <b>Ваша ссылка для приглашений:</b> https://t.me/Football_Inv_Bot?start=${message.from.id} - ссылка для Telegram

https://tgdo.me/Football_Inv_Bot?start=${message.from.id} - ссылка для всех остальных сервисов.
...
Цена создания команды, который может содержать в себе до 10 человек - 65 рублей!
`, {
          parse_mode: "HTML", reply_markup: {
            inline_keyboard: [
              [{ text: "➕ Создать команду (65 рублей)", callback_data: "clan_create" }],
              [{ text: "⚽️ ТОП командов", callback_data: "clan_top" }],
        ...
        if (clan.creator_id == uid || clan.zam_id == uid)
          reply_markup.inline_keyboard.push([{ text: "⚙️ Управление командой", callback_data: "clan_admin" }])
        reply_markup.inline_keyboard.push([{ text: "💳 Пополнить баланс команды", callback_data: "clan_payin" }])
        reply_markup.inline_keyboard.push([{ text: "⚽️ ТОП командов", callback_data: "clan_top" }])
...
<b>Всего игроков в команде:</b> ${members.length + 1} из ${clan.maxMembers}
<b>Доходность команды:</b> ${roundPlus(clan.total_earn)} ⚽️/час
<b>В казне:</b> ${roundPlus(clan.balance)} ⚽️
<b>На балансе:</b> ${roundPlus(clan.bal)}₽\n
<b>Покинуть команду:</b> /leave_clan
`, {
          parse_mode: "HTML", reply_markup
        })
      }
    ...
    if (state[uid] == 1601) {
      if (message.user.buybalance < 65) return message.send(`На Вашем балансе для покупок недостаточно средств для создания команды!`, { parse_mode: "HTML" });
      if ((await Clan.findOne({ name: message.text })) != null) return message.send(`Команда с таким названием уже существует!\nВведите другое название команды:`, { parse_mode: "HTML" });
      state[uid] = undefined
    ...
    if (state[uid] == 160101) {
      ...
      if (!clan) return message.send(`Вы не состоите в команде!`, { parse_mode: "HTML" });
      if (message.user.buybalance < sum) return message.send(`На Вашем балансе для покупок недостаточно средств для пополнения баланса команды!`, { parse_mode: "HTML" });
      state[uid] = undefined
      await User.findOneAndUpdate({ id: uid }, { $inc: { buybalance: -sum } })
      await Clan.findOneAndUpdate({ _id: clan._id }, { $inc: { bal: sum } })
      return message.send(`Баланс команды пополнен на ${sum}₽!`, { reply_markup: { keyboard: $menu, resize_keyboard: true } });
    }
    ...
    if (message.text.startsWith("/invite") && !message.text.startsWith("/invitezam")) {
      ...
      if (members.length > clan.maxMembers - 1) return message.send('В команде закончились места!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
      ...
      if (us.clanName) return message.send('Пользователь уже состоит в команде!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
      await bot.sendMessage(Number(message.text.split(" ")[1]), `<a href="tg://user?id=${uid}">${message.from.first_name}</a> приглашает Вас в команду <b>${clan.name}</b>:`, { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "✅ Подтвердить", callback_data: "clanAccept_" + clan._id }, { text: "❌ Отменить", callback_data: "clanDecline" }]] } })
    ...
    if (message.text.startsWith("/kick")) {
      ...
      if (clan.creator_id != uid && clan.zam_id != uid) return
      if (!us) return message.send('Пользователь не найден в боте!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
      if (us.clanName != clan.name) return message.send('Пользователь не состоит в Вашей команде!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
      ...
      await bot.sendMessage(Number(message.text.split(" ")[1]), `❌ Вы были выгнаны из команды <b>${clan.name}</b>!`, { parse_mode: "html" })
      return message.send(`Вы выгнали <a href="tg://user?id=${Number(message.text.split(" ")[1])}">пользователя</a> из команды!`, { parse_mode: "html", reply_markup: { keyboard: $menu, resize_keyboard: true } });
    }
    if (message.text.startsWith("/invitezam")) {
      ...
      if (clan.creator_id != uid && clan.zam_id != uid) return
      if (!us) return message.send('Пользователь не найден в боте!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
      if (us.clanName != clan.name) return message.send('Пользователь не состоит в Вашем команде!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
      ...
      await bot.sendMessage(us.id, `❗️ Вы стали заместителем админа команды <b>${clan.name}</b>!`, { parse_mode: "html" })
    ...
    if (message.text.startsWith("/removezam")) {
      ...
      await bot.sendMessage(clan.zam_id, `❗️ Вы больше не заместитель админа команды <b>${clan.name}</b>!`, { parse_mode: "html" })
    ...
    if (message.text.startsWith("/leave_clan")) {
      var clan = await Clan.findOne({ name: message.user.clanName })
      if (!message.user.clanName) return message.send('Вы еще не состоите в команде!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
      if (clan.creator_id == uid && !clan.zam_id) return message.send('Вы не можете выйти из своего команды не назначив заместителя!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
        ...
        await bot.sendMessage(clan.zam_id, `❗️ Вы стали администратором команды <b>${clan.name}</b> по причине выхода владельца!`, { parse_mode: "html" })
      }
      ...
      await bot.sendMessage(uid, `❌ Вы вышли из команды <b>${clan.name}</b>!`, { parse_mode: "html" })
      await bot.sendMessage(clan.creator_id, `<a href="tg://user?id=${uid}">Пользователь</a> вышел из Вашей команды!`, { parse_mode: "html" })
    }
...
📥 Инвестировано всего: <b>${Math.round (s.ref)}₽</b>
📤 Выплачено всего: <b>${Math.round (s.fc)}₽</b>
🕐 Старт бота произведен 10.10.2020
...
`, {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: "🚀 Разработчик", url: "https://t.me/works_707" }],
            [{ text: "💬 Чат", url: "https://t.me/chatbotaa" }, {text: "📖 Отзывы", url: "https://t.me/novostiotbotaa" }],
            [{ text: "♻️ Пополнения и выводы", url: "https://t.me/viplatiotbota" }],
            // [{ text: "❓ Помощь", callback_data: "help_main" }],
            [{ text: "💎 Новости проекта", url: "https://t.me/novostiotbotaa" }],
            [{ text: "🥇 Топ инвесторов", callback_data: "topInv" }, { text: "🏆 Топ рефоводов", callback_data: "topRef" }],
            [{ text: "👨‍💻 Администратор", url: "https://t.me/ma584" }],
          ]
        }
      });
    }
...
bot.on('callback_query', async (query) => {
  ...
  if (query.data.startsWith('trees:shop')) {
    ...
    if (id <= maxId + 1) {
      ...
      else var bbtn = [{ text: `🛑 Вы достигли лимита в ${limit} клубов`, callback_data: getNavigationQuery(id + 1, tree.id) }]

    }
    else
      var bbtn = [{ text: `◀️ Сперва купите предыдущий клуб`, callback_data: getNavigationQuery(id - 1, tree.id) }]
...
⚽️ Голов в час: ${tree.earn}`, parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [[
    ...
    return bot.sendMessage(uid, 'Здесь находятся ответы на часто задаваемые вопросы, выберите одну из тем, которая вас интересует:', {
      reply_markup: {
        inline_keyboard: [
          ...
          { text: "💎  Клубы", callback_data: "help_bogs" }],
  ...
  if (query.data.startsWith('help_bogs')) {
    bot.deleteMessage(message.chat.id, message.message_id)
    return bot.sendMessage(uid, '⚽️ <b>Клубы</b>\n\Клубы - это убийцы, которые будут будут приносить Вам доход в виде ⚽️ Голов, которые можно обменять на деньги\nДля сбора Голов, зайдите в раздел «⚽️ Клубы» -> Мои Клубы,\nДля обмена Голов на рубли зайдите в раздел «♻️ Обмен» и произведите обмен', { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "help_main" }]] } })
  }
  ...
  if (query.data.startsWith('store')) {
    bot.deleteMessage(message.chat.id, message.message_id)
    return bot.sendMessage(uid, 'Клубы:', {
      reply_markup: {
        inline_keyboard: [
          [{ text: "⚽️  Купить Клубы", callback_data: "trees:shop0" }],
        //   [{ text: "🤴 Супер Клубы", callback_data: "wildBees" }],
  ...
  if (query.data.startsWith('wildBees')) {
    ...
    wb.map((b) => { reply_markup.inline_keyboard.push([{ text: `🤴 Супер т  анк ${b.level} уровня`, callback_data: "wb_" + b._id }]) })
    ...
    return bot.sendMessage(uid, `🤴 Супер клуб могут появится только благодаря магическому месту\n
<b>Для супер клубов доступно 7 уровней улучшения:</b>\n
Уровень | Голов в час | Стоимость улучшения
1 уровень - 33 ⚽️
2 уровень - 340 ⚽️ - 120₽
3 уровень - 618 ⚽️ - 400₽
4 уровень - 982 ⚽️ - 500₽
...
<b>Ваши супер Клубы:</b>`, {
  ...
  if (query.data.startsWith('wbCollect')) {
    if (message.user.wb_profits == 0)
      return bot.answerCallbackQuery(query.id, '🤴 Ваши супер Клубы пока не принесли дохода!', true);
    else {
      bot.deleteMessage(message.chat.id, message.message_id)
  ...
  if (query.data.startsWith('wbUpLevel_')) {
    ...
    bot.sendPhoto(message.chat.id, "/pin.jpg", {
      caption: `
🤴 <b>Супер Клубы ${wb.level} уровня</b> \n
🕒 <b>В деле:</b> ${Math.floor((Date.now() - wb.start_time) / (1000 * 60 * 60 * 24))} дней
⚽️ <b>Голов  в час:</b> ${wbProfits[wb.level]}
💰 <b>Принёс Голов</b>: ${ wb.bee_profit}`, parse_mode: "HTML", reply_markup
    })
    return bot.answerCallbackQuery(query.id, `🔼 Уровень супер Клуба повышен до ${wb.level} за ${price}₽`, true);
...
⚽️ Принесло <b>${ubm.beesGet}</b> из <b>4</b> супер клубов
`
    }
...
❗️️️ Магическое место - это где появляются супер Клубы
...
⚽️ Магическое место за период своей жизни приносит 4 супер Клуба!\n\n${str}`, {
      reply_markup, parse_mode: "html"
    });
...
💰 Стоимость: ${tree.price}₽
⚽️ Голов в час: ${earn}`, parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [[
  ...
  if (query.data.startsWith('trees:buy')) {
    ...
    if (tree.price > message.user.buybalance) return bot.answerCallbackQuery(query.id, '🚫 Недостаточно денег для покупки.', true);
      ...
      if (treesWithEqualId >= limit)
        return bot.answerCallbackQuery(query.id, `🛑 Вы достигли лимита в ${limit} клуба данного уровня`, true);
...
📥 <b>Для пополнения с помощью других способов обращайтесь к администратору</b> @ma584\n
${bt}`, {
      parse_mode: "HTML",
  ...
  if (query.data == 'game_payin') {
    state[uid] = undefined
    bot.deleteMessage(message.chat.id, message.message_id);
    return bot.sendMessage(message.chat.id, `<b>👇 Выберете способ пополнения игрового баланса:</b>`, { parse_mode: "HTML", reply_markup: { inline_keyboard: [[{ text: "🥝 QIWI", callback_data: "game_payin_qiwi" }], [{ text: "⚽️ Payeer", callback_data: "game_payin_payeer" }], ] } })
  }
...
`, { parse_mode: "HTML", reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "game_payin" }]] } })
  ...
  if (query.data == 'game_payin_payeer') {
    bot.deleteMessage(message.chat.id, message.message_id);
    return bot.sendMessage(message.chat.id, `
⚽️ <b>Способ пополнения: Payeer</b>
...
▫️ Progetto M40 Mod. 65 ▫️ Super Conqueror ▫️ Bat.-Châtillon 25 t ▫️ Объект 430У ▫️ 60TP Lewandowskiego ▫️ Ещё одна прокрутку ▫️ 5₽ ▫️ 10₽ ▫️ 15₽
  ...
  if (query.data == 'game_casino_spin') {
    ...
    if (val <= 5) {
      val--
      var prize = `⚽️ Клуба ${trees.find((a) => a.id == val).name}`
      giveTree(uid, val)
    }
    else if (val == 6) {
...
▫️ <b>Ваш выигрыш:</b> ${prize}`, {
  ...
  if (query.data == 'game_prize') {
    ...
    bot.deleteMessage(message.chat.id, message.message_id);
    return bot.sendMessage(message.chat.id, `🎁 <b>Подарок - Локомотив Москва (р)</b>\n
...
▫️ @works_707`, {
  ...
  if (query.data == 'game_prize_check') {
    var res = await bot.getChatMember("@novostiotbotaa", message.chat.id)
    var res = await bot.getChatMember("@works_707", message.chat.id)
    if (message.user.prize) return bot.answerCallbackQuery(query.id, '🙂 Вы уже получили свой подарок!', true);
    ...
    await message.user.save();
    return bot.sendMessage(message.chat.id, `🎁 <b>Вы получили подарок! Локомотив Москва уже с Вами</b>`, {
      parse_mode: "HTML",
    });
  }
...
//   }
  ...
  if (query.data.startsWith('withdraw:')) {
    ...
    bot.sendMessage("@viplatiotbota", `⚽️ <a href="tg://user?id=${ticket.id}">Пользователь</a> вывел <b>${ticket.amount}₽</b>`, { parse_mode: "HTML" })

    if (ticket.wallet.indexOf("P") == -1) { // Платёж через QIWI
      qiwi.toWallet({ account: String(ticket.wallet), amount: ticket.amount, comment: 'Выплата от @Football_Inv_Bot!' }, () => { });
    }
...
`, {
      parse_mode: "html", reply_markup: {
        inline_keyboard: [
          [{ text: "👨‍💻 Админ", url: "https://t.me/ma584" },
          { text: "💬 Чат", url: "https://t.me/chatbotaa" }],
          [{ text: "💳 Выплаты", url: "https://t.me/viplatiotbota" }],
  ...
  if (d == "clan_create") {
    if (message.user.buybalance < 65)
      return bot.answerCallbackQuery(query.id, `На Вашем балансе для покупок недостаточно средств для создания команды!`, true);
    state[uid] = 1601
    bot.deleteMessage(message.chat.id, message.message_id);
    return bot.sendMessage(message.chat.id, 'Введите название для Вашего команды:', { reply_markup: { keyboard: Cancel, resize_keyboard: true } });
  }
  ...
  if (d == "clan_payin") {
    ...
    state[uid] = 160101
    bot.deleteMessage(message.chat.id, message.message_id);
    return bot.sendMessage(message.chat.id, `💳 <b>Ваш баланс для пополнений:</b> ${message.user.buybalance}₽\nВведите сумму для пополнения баланса Вашего команды:`, { parse_mode: "html", reply_markup: { keyboard: Cancel, resize_keyboard: true } });
  }
  if (d == "clan_admin") {
    var clan = await Clan.findOne({ name: message.user.clanName })
...
Выгнать участника со команды - <code>/kick [id]</code>
...
`, {
  ...
  if (d == "clan_transferMe") {
    ...
    var members = await User.find({ id: { $ne: uid }, clanName: clan.name })
    if (uid != clan.creator_id && uid != clan.zam_id) return bot.answerCallbackQuery(query.id, `Ошибка!`, true);
    if (clan.balance == 0) return bot.answerCallbackQuery(query.id, `Казна команды нулевая!`, true);
    bot.deleteMessage(message.chat.id, message.message_id);
    ...
    return bot.sendMessage(message.chat.id, `<b>${clan.balance} ⚽️ Голов</b> из казны команды начислены Вам как <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для покупок и <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для вывода`, { parse_mode: "html" });
  }
  if (d == "clan_transferAll") {
    ...
    for (const i in members) {
      try {
        ...
        await bot.sendMessage(members[i].id, `<b>${roundPlus(clan.balance)} ⚽️ Голов</b> из казны команды распределены между участниками команды\nВам начислено <b>${roundPlus(dole)}₽</b> на баланс для покупок и <b>${roundPlus(dole)}₽</b> на баланс для вывода`, { parse_mode: "html" });
      }
      catch { }
    }
  }
  if (d == "clan_expand") {
    var clan = await Clan.findOne({ name: message.user.clanName })
    if (uid != clan.creator_id && uid != clan.zam_id) return bot.answerCallbackQuery(query.id, `Ошибка!`, true);
    if (clan.bal < 65) return bot.answerCallbackQuery(query.id, `На балансе команды недотаточно средств!`, true);
    bot.deleteMessage(message.chat.id, message.message_id);
    await Clan.findOneAndUpdate({ name: clan.name }, { $inc: { maxMembers: 10, bal: -65 } })
    await bot.sendMessage(uid, `Вы успешно расширили максимальное количество мест на команды до ${clan.maxMembers + 10}`, { parse_mode: "html" });
  ...
  if (d == "clan_upLevel") {
    ...
    await bot.sendMessage(uid, `Вы успешно повысили доходность казны команды до ${clan.level + 1}%`, { parse_mode: "html" });

  }
  if (d.startsWith("clanAccept")) {
    var clan = await Clan.findOne({ _id: d.split("_")[1] })
    var members = await User.find({ clanName: clan.name })
    if (members.length > clan.maxMembers - 1) return bot.answerCallbackQuery(query.id, `В команде закончились места!`, true);
    bot.deleteMessage(message.chat.id, message.message_id);
    await User.findOneAndUpdate({ id: uid }, { clanName: clan.name })
    await bot.sendMessage(uid, `✅ Вы успешно вступили в команда<b>${clan.name}</b>\nНажмите /leave_clan, чтобы покинуть команду`, { parse_mode: "html" });
    bot.sendMessage(clan.creator_id, `➕ На ваш командавступил <a href="tg://user?id=${uid}">пользователь</a>\nВведите <code>/kick ${uid}</code>, чтобы выгнать участника из команды`, { parse_mode: "html" });
    totalClanEarnCalc()
  }
  if (d == "clanDecline") {
    bot.sendMessage(uid, ` ❌ Вы отменили заявку на вступление во команду!`, { parse_mode: "html" });
  }
...
5⃣ ${clans[4].name} | ${clans[4].total_earn} ⚽️`, { parse_mode: "html" });
      ...
      return bot.sendMessage(message.chat.id, `<b>🏆 Недостаточно Команд для составления топа</b>`, { parse_mode: "html" });
...
🕒 Битва Команд происходит 5, 15 и 25-го числа каждого месяца!
🏆 Команда-победитель получает <b>⚽️ 100k Голов</b> в свою казну
За 2 место - <b>⚽️ 50k Голов</b>
За 3 место - <b>⚽️ 30k Голов</b>
За 4 место - <b>⚽️ 15k Голов</b>
За 5 место - <b>⚽️ 5k Голов</b>

<b>До следующей битвы:</b> ${ost} дней

<b>🛡 ТОП Команд текущей битвы 🛡</b>
...
`, {
      ...
      bot.deleteMessage(message.chat.id, message.message_id);
      bot.sendMessage(uid, 'Введите максимально возможное количество клубов для покупки:', { reply_markup: RM_admin_return, parse_mode: "HTML" })
      state[uid] = 7771222
    }
      ...
      bot.deleteMessage(message.chat.id, message.message_id);
      bot.sendMessage(uid, 'Выберете клуб для выдачи:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "giveTree2_" + d.split("_")[1] + "_0" }], [{ text: trees[1].name, callback_data: "giveTree2_" + d.split("_")[1] + "_1" }], [{ text: trees[2].name, callback_data: "giveTree2_" + d.split("_")[1] + "_2" }], [{ text: trees[3].name, callback_data: "giveTree2_" + d.split("_")[1] + "_3" }], [{ text: trees[4].name, callback_data: "giveTree2_" + d.split("_")[1] + "_4" }], [{ text: trees[5].name, callback_data: "giveTree2_" + d.split("_")[1] + "_5" }], [{ text: trees[6].name, callback_data: "giveTree2_" + d.split("_")[1] + "_6" }], [{ text: trees[7].name, callback_data: "giveTree2_" + d.split("_")[1] + "_7" }], [{ text: trees[8].name, callback_data: "giveTree2_" + d.split("_")[1] + "_8" }]] }, parse_mode: "HTML" })
    }

    else if (d.split("_")[0] == "giveTree2") {
      bot.deleteMessage(message.chat.id, message.message_id);
      giveTree(Number(d.split("_")[1]), Number(d.split("_")[2]))
      bot.sendMessage(Number(d.split("_")[1]), 'Вам выдан клуб: ' + trees[Number(d.split("_")[2])].name, { rparse_mode: "HTML" })
      ...
      bot.sendMessage(uid, 'Выберете клуб, который необходимо отнять:', { reply_markup: keyboard, parse_mode: "HTML" })
    }
      ...
      bot.sendMessage(uid, 'Вы успешно забрали клуб у пользователя!', { reply_markup: { inline_keyboard: [[{ text: "Назад", callback_data: "takeTree_" + id }]] }, parse_mode: "HTML" })
    }

    else if (d == "a_voucher") {
      bot.deleteMessage(message.chat.id, message.message_id);
      bot.sendMessage(uid, 'Выберете клуб для создания чека:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "voucher_0" }], [{ text: trees[1].name, callback_data: "voucher_1" }], [{ text: trees[2].name, callback_data: "voucher_2" }], [{ text: trees[3].name, callback_data: "voucher_3" }], [{ text: trees[4].name, callback_data: "voucher_4" }], [{ text: trees[5].name, callback_data: "voucher_5" }], [{ text: trees[6].name, callback_data: "voucher_6" }], [{ text: trees[7].name, callback_data: "voucher_7" }], [{ text: trees[8].name, callback_data: "voucher_8" }]] }, parse_mode: "HTML" })
    }
    else if (d.split("_")[0] == "voucher") {
      bot.deleteMessage(message.chat.id, message.message_id);
      var cid = generateID(8)
      await Voucher.insertMany({ id: cid, tree_id: Number(d.split("_")[1]) })
      bot.sendMessage(uid, `Чек создан:\nhttps://t.me/Football_Inv_Bot?start=C${cid}`, { reply_markup: RM_admin_return })
    }
    ...
    for (const txnId in body.history) {
      if (lastTxnId == null) { lastTxnId = txnId; console.log(`Last TxnId set to: ${txnId}`) }
        ...
        if (body.history[txnId].comment.startsWith('SG')) {
          ...
          bot.sendMessage("@viplatiotbota", `⚽️ <a href="tg://user?id=${id}">Пользователь</a> пополнил игровой баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
          return
        }
        ...
        if (b == 0) {
          ...
          bot.sendMessage("@viplatiotbota", `⚽️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${sum}₽</b> и получил ${roundPlus(sum * 0.15)}₽ бонусом!`, { parse_mode: "HTML" })
          CHANNEL_INFO.map((a) => bot.sendMessage(a, `<a href="tg://user?id=${id}">Игрок</a> сделал депозит: ${sum}₽ + ${roundPlus(sum * 0.15)}₽ бонус`, { parse_mode: "HTML" }))
        } else {
          await user.inc('deposit', sum);
          b = b / 100
          ...
          bot.sendMessage("@viplatiotbota", `⚽️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${sum}₽</b>`, { parse_mode: "HTML" })
          CHANNEL_INFO.map((a) => bot.sendMessage(a, `<a href="tg://user?id=${id}">Игрок</a> сделал депозит: ${sum}₽ + ${roundPlus(sum * b)}₽ бонус`, { parse_mode: "HTML" }))
        }
  ...
  qiwi.getOperationHistory({ rows: 10, operation: 'IN' }, (err, response) => {
    response.data.map(async (x) => {
      ...
      if (b > 0) {
        ...
        bot.sendMessage("@viplatiotbota", `⚽️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!`, { parse_mode: "HTML" })
        ...
        bot.sendMessage("@viplatiotbota", `⚽️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
...
async function beeMotherUpdater() {
  ...
  for (var i in bm) {
    ...
    bot.sendMessage(b.creator_id, `⚜️ Ваше магическое место принесло 4 супер Клуба и исчезло`)
  }
  // Выдача диких пчёл
  ...
  for (var i in bm) {
    ...
    bot.sendMessage(b.creator_id, `⚽️ Магическое место принесло Вам супер клуба!`)
  }
}
...
💰 Он получает <b>⚽️ 100k Голов</b> в казну команды\n
2 место - <b>${clans[1].name}</b> - получает <b>⚽️ 50k Голов</b>
3 место - <b>${clans[2].name}</b> - получает <b>⚽️ 30k Голов</b>
4 место - <b>${clans[3].name}</b> - получает <b>⚽️ 15k Голов</b>
5 место - <b>${clans[4].name}</b> - получает <b>⚽️ 5k Голов</b>\n
...
async function ma(){
  ...
  response.data.map(async (x) => {
    ...
    if (sum >= 300) {
      ...
      //bot.sendMessage("@Football_Inv_Bot", `⚽️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * 0.15)}₽ бонусом!`, { parse_mode: "HTML" })
      CHANNEL_INFO.map((a) => bot.sendMessage(a, `<a href="tg://user?id=${id}">Игрок</a> сделал депозит: ${x.sum.amount}₽ + ${roundPlus(x.sum.amount * 0.15)}₽ бонус`, { parse_mode: "HTML" }))
      ...
      await User.findOneAndUpdate({ id: 0 }, { $inc: { ref: x.sum.amount } })
      bot.sendMessage(id, `Ваш баланс пополнен на ${x.sum.amount}₽`);
      //bot.sendMessage("@Football_Inv_Bot", `⚽️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
      CHANNEL_INFO.map((a) => bot.sendMessage(a, `<a href="tg://user?id=${id}">Игрок</a> сделал депозит: ${x.sum.amount}₽`, { parse_mode: "HTML" }))
    } else {
      await user.inc('deposit', x.sum.amount);
      if (user.deposit + x.sum.amount > 100 && !user.not) {
      ...
      await User.findOneAndUpdate({ id: 0 }, { $inc: { ref: x.sum.amount } })
      bot.sendMessage(id, `Ваш баланс пополнен на ${x.sum.amount}₽ и Вы получаете бонус - ${roundPlus(x.sum.amount * b)}₽!`);
      //bot.sendMessage("@Football_Inv_Bot", `⚽️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!`, { parse_mode: "HTML" })
      CHANNEL_INFO.map((a) => bot.sendMessage(a, `<a href="tg://user?id=${id}">Игрок</a> сделал депозит: ${x.sum.amount}₽ + ${roundPlus(x.sum.amount * b)}₽ бонус`, { parse_mode: "HTML" }))
